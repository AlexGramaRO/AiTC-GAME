...
def send_monthly_subscription_activation_email(config, to_email, plan_name, start_date, end_date, source='Stripe', manage_url=None):
    ...
    subject = f'Your {label} is active — webATC'
    text_body = (
        f'Your {label} is now active on webATC.\n\n'
        f'Access period: {start_date} to {end_date}\n'
        f'Purchased via: {source}\n\n'
        'You can use the simulator for the full paid period. '
        'Monthly subscriptions renew automatically unless you cancel before the next billing date.\n'
    )
    if manage_url:
    ...
    html_body = _billing_email_html(
        ...
        [
            f'Your <strong>{label}</strong> is now active on webATC.',
...
def send_promo_access_activation_email(config, to_email, promo_code, duration_days, expires_at, manage_url=None):
    subject = 'Promo access activated — webATC'
    text_body = (
        f'Your promo access is now active on webATC.\n\n'
        f'Promotion code: {promo_code}\n'
    ...
    if manage_url:
        text_body += f'\nManage your access: {manage_url}\n'
    html_body = _billing_email_html(
        'Promo access confirmed',
        [
            'Your <strong>promo access</strong> is now active on webATC.',
            f'Promotion code: <strong>{promo_code}</strong>.',
...
def send_subscription_cancellation_email(config, to_email, plan_name, end_date, manage_url=None):
    label = 'Monthly subscription'
    subject = f'{label} cancellation confirmed — webATC'
    text_body = (
        f'Your {label} auto-renewal has been cancelled on webATC.\n\n'
    ...
    if manage_url:
        text_body += f'\nManage your subscription: {manage_url}\n'
    html_body = _billing_email_html(
        'Monthly subscription cancellation confirmed',
        [
            f'Your <strong>{label}</strong> auto-renewal has been cancelled.',
